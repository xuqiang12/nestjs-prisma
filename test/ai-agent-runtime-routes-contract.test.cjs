...
test('agent conversation routes stay available without knowledge-bot prefixes', () => {
  ...
  const knowledgeBotModule = read('src/modules/knowledge-bot/knowledge-bot.module.ts')
  ...
  assert.doesNotMatch(knowledgeBotModule, /ConversationController/)
  assert.doesNotMatch(knowledgeBotModule, /ConversationService/)
