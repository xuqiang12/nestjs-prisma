...
test('v2 stream service delegates real v2 chat flow and keeps entrance errors explicit', () => {
  ...
  assert.match(service, /agentChatService\.stream\([\s\S]*body as AgentStreamRequestDto,[\s\S]*userId,[\s\S]*metadata,[\s\S]*signal,[\s\S]*\)/)
...
test('v2 stream controller only sets SSE headers and delegates event production', () => {
  ...
  assert.match(controller, /const controller = new AbortController\(\)/)
  assert.match(controller, /let completed = false/)
  assert.match(controller, /req\.on\('close'[\s\S]*controller\.abort\(\)/)
  assert.match(controller, /agentStreamService\.stream\(body, req\.user\.userId, controller\.signal\)/)
...
test('agent runtime cancellation uses AbortSignal context without new cancellation abstractions', () => {
  const runtimeTypes = readSource('src/ai-runtime/agent-runtime.types.ts')
  const agentChat = readSource('src/modules/agent-chat/chat/agent-chat.service.ts')
  ...
  const llm = readSource('src/ai-runtime/llm/llm.service.ts')

  assert.match(runtimeTypes, /signal\?: AbortSignal/)
  assert.match(agentChat, /signal\?: AbortSignal/)
  assert.match(agentChat, /signal,/)
  assert.match(runtime, /request\.signal\?\.aborted/)
  assert.match(contextBuilder, /signal: request\.signal/)
  assert.match(llm, /signal\?: AbortSignal/)
  assert.match(llm, /signal: options\.signal/)

  const combined = [runtimeTypes, agentChat, runtime, contextBuilder, llm].join('\n')
  assert.doesNotMatch(combined, /CancellationService|CancellationContext|CancellationManager|ExecutionCancellationState|AbortReason|CancellationEvent/)
})
