...
test('ConversationRepository is the v2 history loader and cleans assistant history', () => {
  ...
  assert.match(repository, /getHistoryMessages\(conversationId\?: string, limit = 20, currentMessage\?: string/)
  ...
  assert.match(repository, /excludeCurrentMessageFromHistory/)
  assert.match(repository, /item\.role === 'user' \|\| item\.role === 'assistant'/)
  assert.match(repository, /item\.role !== 'assistant' \|\| !ROLE_MARKER_PATTERN\.test\(item\.content\)/)
  assert.doesNotMatch(repository, /private isCleanAssistantHistoryContent/)
...
test('AgentContextBuilder resolves agent config through one v2 entrypoint', () => {
  ...
  assert.match(builder, /conversationRepository\.getHistoryMessages\(\s*request\.conversation\.id,\s*20,\s*request\.message\.content,\s*\)/)
