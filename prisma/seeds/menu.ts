...
const menuSeeds: MenuSeed[] = [
  ...
  },
  {
    ...
    sort: 0,
    type: MenuType.MENU,
    parentPath: '/AIEngine/application',
  },
  {
    name: '知识库管理',
...
const buttonSeeds: ButtonSeed[] = [
  ...
  { menuPath: '/AIEngine/model/index', permissionCode: 'ai:model:list', name: '查询', sort: 1 },
  { menuPath: '/AIEngine/model/index', permissionCode: 'ai:model:add', name: '新增', sort: 2 },
  { menuPath: '/AIEngine/model/index', permissionCode: 'ai:model:update', name: '修改', sort: 3 },
  { menuPath: '/AIEngine/model/index', permissionCode: 'ai:model:status', name: '启停', sort: 4 },
...
export async function seedMenus(permissions: SeedPermission[] = [], roleId?: string) {
  ...
  if (roleId) {
    ...
    const modelPermissionIds = permissions
      .filter((item) => item.code.startsWith('ai:model:'))
      .map((item) => item.id)
    await prisma.rolePermission.createMany({
      data: modelPermissionIds.map((permissionId) => ({
        roleId,
        permissionId,
      })),
      skipDuplicates: true,
