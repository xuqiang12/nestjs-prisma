...
describe('HomeController (e2e)', () => {
  ...
  it('saves homepage components as one ordered list and soft deletes removed components', async () => {
    ...
    expect(saveResponse.body.data.map((item) => item.templateId)).toEqual(['homePageTitle', 'advert', 'search'])
    ...
    await request(app.getHttpServer())
      ...
      .expect(200)
      .expect(({ body }) => {
        expect(body.data.components.map((item) => item.templateId)).toEqual(['homePageTitle', 'advert', 'search'])
        expect(body.data.components[0]).toMatchObject({
          templateId: 'homePageTitle',
          templateName: '首页头部',
          info: titleInfo,
